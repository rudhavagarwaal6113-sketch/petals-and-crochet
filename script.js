...
let products = [], cart = JSON.parse(localStorage.getItem('petals_cart') || '[]'), user = null, profile = null, category = 'all', authMode = 'login';
const placeholders = ['🌷','🌹','🧸','🧶','👜','💗','🔑','🌷'];
...
function saveCart(){localStorage.setItem('petals_cart',JSON.stringify(cart));renderCart()}
...
function cartRows(){return cart.map(row=>{const p=products.find(x=>x.id===row.id);return p?{...row,p}:null}).filter(Boolean)}
function renderCart(){if(!$('#cartCount'))return;const rows=cartRows();$('#cartCount').textContent=rows.reduce((a,x)=>a+x.qty,0);$('#cartItems').innerHTML=rows.length?rows.map(x=>{const im=img(x.p);return `<div class="cart-row"><div>${im?`<img src="${safe(im)}">`:'🌷'}</div><div><h4>${safe(x.p.name)}</h4><div>${money(x.p.price)}</div><div class="qty"><button onclick="changeQty(${x.id},-1)">−</button><span>${x.qty}</span><button onclick="changeQty(${x.id},1)">+</button></div></div><strong>${money(x.p.price*x.qty)}</strong></div>`}).join(''):'<div class="empty">Your bag is waiting for something lovely.</div>';const sub=rows.reduce((a,x)=>a+x.p.price*x.qty,0);$('#cartSubtotal').textContent=money(sub);$('#giftNote').textContent=sub>=599?'🎁 Your free gift is included!':`Add ${money(599-sub)} more for a free gift.`;$('#checkoutBtn').disabled=!rows.length}
window.changeQty=function(id,d){const r=cart.find(x=>x.id===id);if(!r)return;r.qty+=d;const p=products.find(x=>x.id===id);if(r.qty<1)cart=cart.filter(x=>x.id!==id);else if(p&&r.qty>p.stock)r.qty=p.stock;saveCart()}
function openPanel(id){$('#'+id).classList.add('open');$('#overlay').classList.add('show')}
function closePanel(id){$('#'+id).classList.remove('open');if(!$$('.side-panel.open').length)$('#overlay').classList.remove('show')}
function closeModal(id){$('#'+id).classList.add('hidden')}
window.closeModal=closeModal;
$$('[data-close]').forEach(b=>b.onclick=()=>{const id=b.dataset.close;if(id.includes('Modal'))closeModal(id);else closePanel(id)});
$('#overlay').onclick=()=>{closePanel('cartPanel');closePanel('accountPanel')};
...
async function renderAdminProducts(){const {data,error}=await db.from('products').select('*').order('created_at',{ascending:false});if(error)return toast(error.message);const rows=data||[];$('#adminContent').innerHTML=`<div class="admin-form"><h3>Add product</h3><form id="productForm"><div class="row"><input name="name" placeholder="Product name" required><input name="category" placeholder="bouquets / plushies / accessories" required></div><div class="row"><input name="price" type="number" min="0" step="1" placeholder="Price ₹" required><input name="compare" type="number" min="0" step="1" placeholder="Compare-at price"></div><div class="row"><input name="stock" type="number" min="0" placeholder="Stock" required><input name="image" type="url" placeholder="Image URL (optional)"></div><textarea name="description" placeholder="Description"></textarea><label><input name="featured" type="checkbox"> Featured product</label><label>Upload image <input name="file" type="file" accept="image/*"></label><button class="btn dark" type="submit">Add product</button></form></div><h3>Current products</h3>${rows.length?rows.map(p=>`<div class="admin-product"><div>${img(p)?`<img src="${safe(img(p))}">`:'🌷'}</div><div><strong>${safe(p.name)}</strong><br><small>${money(p.price)} · stock ${p.stock} · ${p.active?'live':'hidden'}</small></div><div class="admin-product-actions"><button onclick="toggleProduct(${p.id},${!p.active})">${p.active?'Hide':'Show'}</button><button onclick="deleteProduct(${p.id})">Delete</button></div></div>`).join(''):'<p>No products yet.</p>'}`;$('#productForm').onsubmit=addProduct}
async function addProduct(e){e.preventDefault();const f=new FormData(e.target);let image=f.get('image')?.trim()||null;const file=f.get('file');if(file&&file.size){const ext=file.name.split('.').pop();const path=`${crypto.randomUUID()}.${ext}`;const up=await db.storage.from('product-images').upload(path,file,{upsert:false});if(up.error)return toast(up.error.message);image=db.storage.from('product-images').getPublicUrl(path).data.publicUrl}const base={name:f.get('name'),slug:slugify(f.get('name'))+'-'+Date.now(),category:f.get('category'),description:f.get('description'),price:Number(f.get('price')),compare_at_price:f.get('compare')?Number(f.get('compare')):null,image_url:image,stock:Number(f.get('stock')),active:true,featured:f.get('featured')==='on'};const {error}=await db.from('products').insert([base]);if(error)return toast(error.message);toast('Product added ✿');await loadProducts();renderAdminProducts()}
window.toggleProduct=async(id,value)=>{const {error}=await db.from('products').update({active:value,updated_at:new Date().toISOString()}).eq('id',id);if(error)toast(error.message);else{toast(value?'Product live':'Product hidden');await loadProducts();renderAdminProducts()}}
